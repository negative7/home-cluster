...
resource "cloudflare_filter" "countries" {
  ...
  description = "Expression to block all countries except US, CA, AU and UA"
  expression  = "(ip.geoip.country ne \"US\" and ip.geoip.country ne \"CA\" and ip.geoip.country ne \"AU\" and ip.geoip.country ne \"UA\")"
}

resource "cloudflare_firewall_rule" "countries" {
  zone_id     = lookup(data.cloudflare_zones.domain.zones[0], "id")
  description = "Firewall rule to block all countries except US, CA, AU and UA"
